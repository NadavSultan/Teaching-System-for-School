...
Phase 30 adds one reviewed additive migration for source versions, independent review/permission evidence, durable ingestion, immutable knowledge items, exact curriculum lineage, and PostgreSQL lexical search. Validate it from zero on both the test and clean databases, then run the isolated shadow migration-history comparison. The hand-authored SQL includes visibility checks, immutable provenance triggers, exact published curriculum-link triggers, and the knowledge search index beyond the Prisma client datamodel.
